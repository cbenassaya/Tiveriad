...
public class MemoryPersistenceProvider : ISingletonMemoryProvider
{
        ...
        DateTime? createdFrom, DateTime? createdTo, int skip, int take)
    {
        lock (_instances)
        {
            ...
            if (status.HasValue) result = result.Where(x => x.Status == status.Value);

            if (!string.IsNullOrEmpty(type)) result = result.Where(x => x.WorkflowDefinitionId == type);

            if (createdFrom.HasValue) result = result.Where(x => x.CreateTime >= createdFrom.Value);

            if (createdTo.HasValue) result = result.Where(x => x.CreateTime <= createdTo.Value);
        ...
        CancellationToken _ = default)
    {
        lock (_subscriptions)
        {
            var result = _subscriptions
                .FirstOrDefault(x =>
                    x.ExternalToken == null && x.EventName == eventName && x.EventKey == eventKey &&
    ...
    public async Task MarkEventUnprocessed(string id, CancellationToken _ = default)
    {
        ...
        {
            var evt = _events.FirstOrDefault(x => x.Id == id);
            if (evt != null) evt.IsProcessed = false;
        }
    }
