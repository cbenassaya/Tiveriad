...
public class WorkflowConsumer : QueueConsumer, IBackgroundTask
{
    ...
    public WorkflowConsumer(IPersistenceProvider persistenceProvider, IQueueProvider queueProvider,
        ILoggerFactory loggerFactory, IServiceProvider serviceProvider, IWorkflowRegistry registry,
        IDistributedLockProvider lockProvider, IWorkflowExecutor executor, IDateTimeProvider datetimeProvider,
        IGreyList greylist, IWorkflowOptions options)
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var events = await persistenceStore.GetEvents(subscription.EventName, subscription.EventKey,
                subscription.SubscribeAsOf, cancellationToken);
            ...
            foreach (var evt in events)
            {
                ...
                {
                    if (acquiredLock) await _lockProvider.ReleaseLock(eventKey);
                }
            }
        }
    }
    ...
    private async void FutureQueue(WorkflowInstance workflow, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (target > 0) await Task.Delay(TimeSpan.FromTicks(target), cancellationToken);
