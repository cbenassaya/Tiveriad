...
public class RunnablePoller : IBackgroundTask
{
    ...
    public RunnablePoller(IPersistenceProvider persistenceStore, IQueueProvider queueProvider,
        ILoggerFactory loggerFactory, IServiceProvider serviceProvider, IWorkflowRegistry registry,
        IDistributedLockProvider lockProvider, IGreyList greylist, IDateTimeProvider dateTimeProvider,
        ...
        try
        {
            ...
            if (await _lockProvider.AcquireLock("poll-commands", new CancellationToken()))
                try
                {
                    ...
                    await _persistenceStore.ProcessCommands(new DateTimeOffset(_dateTimeProvider.UtcNow),
                        async command =>
                        {
