...
public class EventConsumer : QueueConsumer, IBackgroundTask
{
    ...
    public EventConsumer(IWorkflowRepository workflowRepository, ISubscriptionRepository subscriptionRepository,
        IEventRepository eventRepository, IQueueProvider queueProvider, ILoggerFactory loggerFactory,
        IServiceProvider serviceProvider, IWorkflowRegistry registry, IDistributedLockProvider lockProvider,
        IWorkflowOptions options, IDateTimeProvider datetimeProvider, IGreyList greylist)
    ...
    protected override async Task ProcessItem(string itemId, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (evt.EventTime <= _datetimeProvider.UtcNow)
            {
                ...
                {
                    var activity =
                        await _subscriptionRepository.GetSubscription((evt.EventData as ActivityResult).SubscriptionId,
                            cancellationToken);
                    if (activity == null)
                    {
                        Logger.LogWarning(
                            $"Activity already processed - {(evt.EventData as ActivityResult).SubscriptionId}");
    ...
    private async Task<bool> SeedSubscription(Event evt, EventSubscription sub, HashSet<string> toQueue,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(sub.ExecutionPointerId))
                pointers = workflow.ExecutionPointers.Where(p =>
                    p.Id == sub.ExecutionPointerId && !p.EventPublished && p.EndTime == null);
            else
                pointers = workflow.ExecutionPointers.Where(p =>
