...
public class WorkflowExecutor : IWorkflowExecutor
{
    ...
    public WorkflowExecutor(IWorkflowRegistry registry, IServiceProvider serviceProvider, IScopeProvider scopeProvider,
        IDateTimeProvider datetimeProvider, IExecutionResultProcessor executionResultProcessor,
        ILifeCycleEventPublisher publisher, ICancellationProcessor cancellationProcessor, IWorkflowOptions options,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (def == null)
        {
            _logger.LogError("Workflow {0} version {1} is not registered", workflow.WorkflowDefinitionId,
                workflow.Version);
            return wfResult;
        }
        ...
        foreach (var pointer in exePointers)
        {
            ...
            {
                _logger.LogError(ex, "Workflow {0} raised error on step {1} Message: {2}", workflow.Id, pointer.StepId,
                    ex.Message);
                wfResult.Errors.Add(new ExecutionError
                {
                    WorkflowId = workflow.Id,
        ...
        WorkflowDefinition def, ExecutionPointer pointer)
    {
        ...
        if (!pointer.StartTime.HasValue) pointer.StartTime = _datetimeProvider.UtcNow;
    ...
    private async Task DetermineNextExecutionTime(WorkflowInstance workflow, WorkflowDefinition def)
    {
        ...
        if (workflow.Status == WorkflowStatus.Complete) return;
        ...
        if (workflow.NextExecution != null || workflow.ExecutionPointers.Any(x => x.EndTime == null)) return;
