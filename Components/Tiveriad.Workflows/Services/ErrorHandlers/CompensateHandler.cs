...
public class CompensateHandler : IWorkflowErrorHandler
{
    ...
    public void Handle(WorkflowInstance workflow, WorkflowDefinition def, ExecutionPointer exceptionPointer,
        WorkflowStep exceptionStep, Exception exception, Queue<ExecutionPointer> bubbleUpQueue)
    {
        ...
        while (scope.Any())
        {
            ...
            if (scopeStep.CompensationStepId.HasValue)
            {
                ...
                var nextCompensationPointer = _pointerFactory.BuildCompensationPointer(def, scopePointer,
                    exceptionPointer, scopeStep.CompensationStepId.Value);
                ...
                if (resume)
                    foreach (var outcomeTarget in scopeStep.Outcomes.Where(x => x.Matches(workflow.Data)))
                        workflow.ExecutionPointers.Add(
                            _pointerFactory.BuildNextPointer(def, scopePointer, outcomeTarget));
            }

            if (revert)
            {
                var prevSiblings = workflow.ExecutionPointers
                    .Where(x => scopePointer.Scope.SequenceEqual(x.Scope) && x.Id != scopePointer.Id &&
                ...
                foreach (var siblingPointer in prevSiblings)
                {
                    ...
                    {
                        var nextCompensationPointer = _pointerFactory.BuildCompensationPointer(def, siblingPointer,
                            exceptionPointer, siblingStep.CompensationStepId.Value);
                        if (compensationPointer != null)
                        {
                            nextCompensationPointer.Active = false;
                            nextCompensationPointer.Status = PointerStatus.PendingPredecessor;
