...

{
    ...
    public void Subscribe<TEvent,TKey>(Func<TEvent, Task> handler, Func<TEvent, Task<bool>> filter = null, Func<Exception, TEvent, Task> onError = null)
        ...
        where TKey : IEquatable<TKey>
    {
        ...
        var handlerInstances  = _serviceResolver.GetServices(typeof(IEventHandler<TEvent,TKey>)).Select(x => x).Cast<IEventHandler<TEvent,TKey>>().ToArray();
    ...
    private async Task TryReportError<TEvent,TKey>(Exception exception, IEventHandler<TEvent,TKey> handler, TEvent @event)
        where TEvent:IDomainEvent < TKey> 
        where TKey : IEquatable<TKey>
    {
        try
