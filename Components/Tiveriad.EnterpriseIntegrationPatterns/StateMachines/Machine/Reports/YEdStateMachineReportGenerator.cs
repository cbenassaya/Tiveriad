...
namespace Tiveriad.EnterpriseIntegrationPatterns.StateMachines.Machine.Reports;
    ...
    where TEvent : IComparable
{
    ...
    public void Report(string name, IEnumerable<IStateDefinition<TState, TEvent>> states, TState initialStateId)
    {
        ...
        NullGuard.AgainstNullArgument("states", statesList);
        ...
        var doc = new XDocument(
            new XElement(
                ...
                new XElement(N + "key", new XAttribute("attr.name", "Beschreibung"),
                    new XAttribute("attr.type", "string"), new XAttribute("for", "graph"), new XAttribute("id", "d7"),
                    new XElement(N + "default")),
                new XElement(N + "key", new XAttribute("attr.name", "url"), new XAttribute("attr.type", "string"),
    ...
    {
        foreach (var state in states.Where(s => s.SuperState == null)) AddNode(graph, state);
    }
    ...
    private void AddNode(XElement graph, IStateDefinition<TState, TEvent> state)
    {
        ...
        if (state.SubStates.Any())
        {
            ...
            if (initialState) groupNode.Add(new XElement(Y + "BorderStyle", new XAttribute("width", "2.0")));

            var subGraph = new XElement(N + "graph", new XAttribute("edgedefault", "directed"),
                new XAttribute("id", state.Id + ":"));
            node.Add(subGraph);
            foreach (var subState in state.SubStates) AddNode(subGraph, subState);
        }
        else
        {
            ...
            if (initialState) shape.Add(new XElement(Y + "BorderStyle", new XAttribute("width", "2.0")));
    ...
    {
        return state.Id.ToString() == initialState.ToString()
