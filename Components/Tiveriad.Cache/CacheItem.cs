...

{
    ...
    private CacheItem(string key, string region, T value, ExpirationMode? expiration, TimeSpan? timeout,
    ...
    {
        return new(Key, Region, Value, mode, timeout, mode == ExpirationMode.Absolute ? DateTime.UtcNow : CreatedUtc,
            LastAccessedUtc, usesHandleDefault);
    }

    /// <summary>
    ///     Creates a copy of the current cache item and sets a new absolute expiration date.
    ///     This method doesn't change the state of the item in the cache. Use <c>Put</c> or similar methods to update the
    ///     cache with the returned copy of the item.
    /// </summary>
    ...
    {
        return new(Key, Region, Value, ExpirationMode.None, TimeSpan.Zero, CreatedUtc, LastAccessedUtc, false);
    }

    /// <summary>
    ///     Creates a copy of the current cache item with no explicit expiration, instructing the cache to use the default
    ///     defined in the cache handle configuration.
    ///     This method doesn't change the state of the item in the cache. Use <c>Put</c> or similar methods to update the
    ///     cache with the returned copy of the item.
    /// </summary>
    /// <remarks>We do not clone the cache item or value.</remarks>
    ...
    {
        return new(Key, Region, Value, ExpirationMode.Default, TimeSpan.Zero, CreatedUtc, LastAccessedUtc);
    }
    ...
    {
        return new(Key, Region, value, ExpirationMode, ExpirationTimeout, CreatedUtc, LastAccessedUtc,
    ...
    {
        return new(Key, Region, Value, ExpirationMode, ExpirationTimeout, created, LastAccessedUtc,
            UsesExpirationDefaults);
    }
}