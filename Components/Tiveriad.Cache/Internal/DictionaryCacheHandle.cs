    ...
    public class DictionaryCacheHandle<TCacheValue> : BaseCacheHandle<TCacheValue>
    {
        ...
        private void TimerLoop(object state)
        {
            ...
            if (Interlocked.CompareExchange(ref _scanRunning, 1, 0) == 0)
            {
                try
                {
                    _logger.LogDebug("'{0}' starting eviction scan.", Configuration.Name);

                    ScanForExpiredItems();
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Error occurred during eviction scan.");
                }
                finally
                {
        ...
        private int ScanForExpiredItems()
        {
            ...
            if (removed > 0 )
            {
                _logger.LogInformation("'{0}' removed '{1}' expired items during eviction run.", Configuration.Name, removed);
            }
