...
public abstract class BaseCacheHandle<TCacheValue> : BaseCache<TCacheValue>, IDisposable
{
    ...
    ///     <para>
    ///         If the cache handle is distributed, a remote remove event for example does not cause another <c>Remove</c>
    ///         call.
    ...
    /// <summary>
    ///     Can be used to signal a remove event to the <see cref="ICacheManager{TCacheValue}" /> in case the underlying cache
    ///     supports this and the implementation
    ...
    /// <param name="value">
    ///     The original cache value. The value might be null if the underlying cache system doesn't support
    ///     returning the value on eviction.
    /// </param>
    ...
    protected void TriggerCacheSpecificRemove(string key, string region, CacheItemRemovedReason reason, object value)
    {
        ...
        _logger.LogDebug("'{0}' triggered remove '{1}:{2}' because '{3}'.", Configuration.Name, region, key, reason);
