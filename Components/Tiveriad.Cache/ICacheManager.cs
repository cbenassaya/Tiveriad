...
public interface ICacheManager<TCacheValue> : ICache<TCacheValue>
{
    ...
    ///     The <see cref="CacheItemRemovedEventArgs.Reason" /> property indicates the reason while the
    ///     <see cref="CacheItemRemovedEventArgs.Level" /> indicates
    ...
    /// <returns>
    ///     <c>True</c> if the operation succeeds, <c>False</c> in case there are too many retries or the
    ///     <paramref name="valueFactory" /> returns null.
    /// </returns>
    /// <exception cref="ArgumentException">
    ///     If either <paramref name="key" /> or <paramref name="valueFactory" /> is null.
    /// </exception>
    ...
    ///     If either <paramref name="key" /> or <paramref name="valueFactory" /> is null.
    /// </exception>
    bool TryGetOrAdd(string key, string region, Func<string, string, CacheItem<TCacheValue>> valueFactory,
    ...
    /// </summary>
    /// <remarks>
    ///     Don't use this in concurrency critical scenarios if you are using distributed caches as <code>Expire</code> is not
    ///     atomic;
    ///     <code>Expire</code> uses <see cref="ICache{TCacheValue}.Put(CacheItem{TCacheValue})" /> to store the item with the
    ///     new expiration.
    /// </remarks>
    /// <param name="key">The cache key.</param>
    ...
    /// <summary>
    ///     Explicitly sets an absolute expiration date for the <paramref name="key" /> in <paramref name="region" /> in all
    ///     cache layers.
    ///     This operation overrides any configured expiration per cache handle for this particular item.
    /// </summary>
    /// <remarks>
    ...
    /// <summary>
    ///     Explicitly sets a sliding expiration date for the <paramref name="key" /> in <paramref name="region" /> in all
    ///     cache layers.
    ///     This operation overrides any configured expiration per cache handle for this particular item.
    /// </summary>
    /// <remarks>
    ...
    /// <summary>
    ///     Explicitly removes any expiration settings previously defined for the <paramref name="key" /> in
