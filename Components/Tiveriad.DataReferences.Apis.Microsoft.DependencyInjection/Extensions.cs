...
public static class DataReferencesExtensions
{
    ...
    private static HandlerDefinition[] _handlerDefinitions = new[]
    {
    ...
    private static void AddEntity(this IServiceCollection services,HandlerDefinition  handlerDefinition, Type type, Type keyType)
    {
        var typeParameters = new[] { type, keyType };
        Type responseType = null;
        var requestType = handlerDefinition.RequestType.MakeGenericType(typeParameters);

        if (handlerDefinition.ResponseType.Equals(typeof(MediatR.Unit)))
            responseType = type;
