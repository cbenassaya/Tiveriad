...
public class UpdateOrganizationPreValidator : AbstractValidator<UpdateOrganizationRequest>
{
    ...
    {
        RuleFor(x => x.Organization).NotNull().WithMessage("Organization is required").WithErrorCode("IDENTITIES_ORGANIZATION_UPDATE_ORGANIZATION_REQUIRED");
        RuleFor(x => x.Organization.Name).NotEmpty().WithMessage("Name is required").WithErrorCode("IDENTITIES_ORGANIZATION_UPDATE_NAME_REQUIRED");
        RuleFor(x => x.Organization.Id).NotEmpty().WithMessage("Id is required").WithErrorCode("IDENTITIES_ORGANIZATION_UPDATE_ID_REQUIRED");
        ...
        RuleFor(x => x).Must( request =>
        {
            ...
            return organization == null;
        }).WithMessage("Organization name already exists").WithErrorCode("IDENTITIES_ORGANIZATION_UPDATE_NAME_ALREADY_EXISTS");
