...
public class UpdateClientPreValidator : AbstractValidator<UpdateClientRequest>
{
    ...
    {
        RuleFor(x => x.Client).NotNull().WithMessage("Client is required")
            .WithErrorCode("IDENTITIES_CLIENT_UPDATE_CLIENT_REQUIRED");
        RuleFor(x => x.Client.Name).NotEmpty().WithMessage("Name is required")
            .WithErrorCode("IDENTITIES_CLIENT_UPDATE_NAME_REQUIRED");
        RuleFor(x => x.Client.Code).NotEmpty().WithMessage("Code is required")
            .WithErrorCode("IDENTITIES_CLIENT_UPDATE_CODE_REQUIRED");
        RuleFor(x => x.OrganizationId).NotEmpty().WithMessage("OrganizationId is required")
            .WithErrorCode("IDENTITIES_CLIENT_UPDATE_ORGANIZATIONID_REQUIRED");
        ...
        RuleFor(x => x).Must(request =>
        {
            var client = clientRepository
                .Queryable
                .FirstOrDefault(c =>
                    c.Name == request.Client.Name && c.Id != request.Client.Id &&
                    c.Organization.Id == request.OrganizationId);
