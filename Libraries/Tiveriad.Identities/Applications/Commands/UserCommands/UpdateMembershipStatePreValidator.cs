...
public class UpdateMembershipStatePreValidator : AbstractValidator<UpdateMembershipStateRequest>
{
        ...
        IRepository<Membership, string> membershipRepository)
    {
        RuleFor(x => x.OrganizationId)
            .NotNull().NotEmpty()
            .WithMessage("OrganizationId cannot be null or empty").WithErrorCode("UPDATE_USER_ORGANIZATION_ID_REQUIRED");
        RuleFor(x => x.OrganizationId)
            .Must((id) =>
            {
        ...
        RuleFor(x => x.membershipState)
            .NotNull().NotEmpty()
            .WithMessage("MembershipState cannot be null or empty").WithErrorCode("UPDATE_USER_MEMBERSHIP_STATE_REQUIRED");
        
        RuleFor(x => x.membershipState)
            .Must((state) =>
            {
                return Enum.TryParse<MembershipState>(state, out var _);
        ...
        RuleFor(x => x)
            .Must((request) =>
            {
                var query = membershipRepository.Queryable
                    ...
                    .Include(x => x.User)
                    .Include(x => x.Client)
                    .Where(x => x.Organization.Id == request.OrganizationId && x.User.Id == request.userId && x.Client.Id == request.ClientId);
                return query.FirstOrDefault() != null;
