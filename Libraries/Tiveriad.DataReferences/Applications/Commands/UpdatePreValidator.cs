    ...
    where TKey : IEquatable<TKey>
{
    ...
    public UpdatePreValidator(IRepository<TEntity, TKey> repository)
    {
        ...
        }).WithMessage($"{typeof(TEntity).Name} doesn't exist").WithErrorCode(typeof(TEntity).Name + "_DOES_NOT_EXIST");
        ...
        RuleFor(x => x.Entity.Label).NotEmpty().WithMessage("Locale is required").WithErrorCode(typeof(TEntity).Name + "_LOCALE_IS_REQUIRED");
        RuleFor(x => x.Entity).MustAsync(async (entity, cancellationToken) =>
        {
