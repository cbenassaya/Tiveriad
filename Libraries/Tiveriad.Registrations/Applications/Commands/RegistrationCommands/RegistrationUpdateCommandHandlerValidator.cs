...
public class RegistrationUpdateCommandHandlerValidator : AbstractValidator<RegistrationUpdateCommandHandlerRequest>
{
    private IRepository<Registration, string> _repository;
    public RegistrationUpdateCommandHandlerValidator(IRepository<Registration, string> repository)
    {
        _repository = repository;

        RuleFor(x => x.Registration).NotNull().WithErrorCode(ErrorCodes.RegistrationUpdateCommandHandler_Registration_XNotNullRule);
        RuleFor(x => x.Registration.Id).NotNull().WithErrorCode(ErrorCodes.Registration_Id_XNotNullRule);
